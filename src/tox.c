...
static void log_callback(Tox *UNUSED(tox), TOX_LOG_LEVEL UNUSED(level), const char *UNUSED(file),
                         uint32_t UNUSED(line), const char *UNUSED(func),
                         const char *UNUSED(message), void *UNUSED(user_data))
{
}
    ...
    switch (msg) {
        ...
        case TOX_FRIEND_NEW: {
            ...
            if (!param1) {
                ...
                fid = tox_friend_add(tox, data, (const uint8_t *)default_add_msg->str,
                                     default_add_msg->length, &f_err);
            } else {
        ...
        case TOX_SEND_ACTION: {
            ...
            mmsg->receipt      = tox_friend_send_message(tox, param1, type, next, param2, NULL);
            mmsg->receipt_time = 0;
        ...
        case TOX_CALL_RESUME_VIDEO: {
            ...
            get_friend(param1)->call_state_self |= TOXAV_FRIEND_CALL_STATE_SENDING_V
